...
- **v2 text cleanup is a shared optional pipeline step.** `TextCleanupService` first normalizes input to Unicode Form C, then rejoins hyphenated fragments only across single line breaks, preserves paragraph breaks as one newline, and handles whitespace cleanup before `WordSplitter` runs. It does not use in-band sentinel characters, so existing control characters in user text are preserved. `TextPipeline` returns raw text, processed text, and words so capture, import, and history replay can share one processing path while respecting the cleanup toggle. (2026-04-28)
